@if($catalog->status === \App\Enums\CatalogStatus::EDITING()->getValue())
    <span class="tag is-light is-warning">{{ trans('labels.catalog.status.' . $catalog->status) }}</span>
@elseif($catalog->status === \App\Enums\CatalogStatus::SEALED()->getValue())
    <span class="tag is-light is-success">{{ trans('labels.catalog.status.' . $catalog->status) }}</span>
@elseif($catalog->status === \App\Enums\CatalogStatus::OBSOLETE()->getValue())

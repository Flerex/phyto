        ...
        <table class="table is-fullwidth">
            ...
            @foreach($task->processes as $process)
                <tr>
                    ...
                    <td>
                        <progress class="progress is-link is-small" value="{{ $process->completenessPercentage }}" max="100">{{ $process->completenessPercentage }}%</progress>
                    </td>
                    <td class="has-text-right">
                        <a href="{{ route('panel.projects.tasks.show_process', compact('project', 'task', 'process')) }}" class="button is-link is-rounded is-light is-small has-text-weight-bold">@lang('general.view')</a>
                    </td>
                </tr>
            @endforeach
            </tbody>
